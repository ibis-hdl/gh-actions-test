...
conan_install()
{
    ...
    conan install . --settings build_type=Release --build=missing --profile:all=${conan_profile}
    conan install . --settings build_type=Debug   --build=missing --profile:all=${conan_profile}
}

conan_install_linux_default() {
...
conan_install_linux_clang_libcpp() {
    local build_type=release
